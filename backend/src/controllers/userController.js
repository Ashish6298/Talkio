...
const getProfile = async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(userId)
      .select("username email profilePic bio friends sentRequests receivedRequests") // Added bio
      .populate("friends", "username profilePic")
      .lean();
    ...
    const profile = {
      ...
      email: user.email,
      profilePic: user.profilePic,
      bio: user.bio || '', // Include bio, default to empty string if undefined
      numberOfFriends: user.friends.length,
      friends: user.friends,
      sentRequestsCount: user.sentRequests.length,
      receivedRequestsCount: user.receivedRequests.length,
...
const updateBio = async (req, res) => {
  ...
  try {
    ...
    if (bio.length > 150) {
      return res
        .status(400)
        .json({ success: false, message: "Bio cannot exceed 150 characters" });
    }
