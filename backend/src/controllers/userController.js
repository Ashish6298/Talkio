...
const getUsers = async (req, res) => {
  ...
  try {
    ...
    const currentUser = await User.findById(userId).select('friends sentRequests receivedRequests');
    if (!currentUser) {
...
const sendFriendRequest = (io) => async (req, res) => {
  ...
  try {
    ...
    io.to(senderId).emit("friendRequestSent", { receiverId, receiverUsername: receiver.username });
...
const acceptFriendRequest = (io) => async (req, res) => {
  ...
  try {
    ...
    io.to(senderId).emit("friendRequestAccepted", { userId: receiverId, username: receiver.username });
    io.to(receiverId).emit("friendRequestAccepted", { userId: senderId, username: sender.username });
...
module.exports = { getUsers, sendFriendRequest, acceptFriendRequest };